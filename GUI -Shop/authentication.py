...
def register():
    ...
    frame.create_text(100, 250, text="e-mail:")
    ...
    frame.create_window(200, 250, window=e_mail_box)
    ...
    frame.create_window(350, 450, window=registration_btn)
...
def check_registration(info):
    ...
    if info["e-mail"].count('@') > 1:
        frame.create_text(
            300,
            320,
            text="Email should contain only one @",
            fill="red",
            tag="error",
        )


    if info["e-mail"].split('.')[-1] not in valid_domains:
        frame.create_text(
            300,
            360,
            text="Domain must be one of the folowing: .com, .bg, .org, .net",
            fill="red",
            tag="error",
        )
        return False

    if len(info["password"][0]) < 6:
        frame.create_text(
            300,
            300,
